...
- Agustina Salatino (`agustinasalatino`) — `killer_queries.py`, `docs-resultados/resultados_killer_queries.md`
...
**A.4 — Índice FAISS.** Repartido en `similitud_coseno.py` (embeddings locales), `construir_indice_faiss.py` (arma y persiste con `write_index()`) y `pipeline_vectorial.py` (recarga con `read_index()`, busca con umbral). Log fijo de 3 consultas de prueba con score y distancia coseno en `docs-resultados/resultados_a4.md`, generado por `reporte_a4.py`.
...
**B.5 — ETL y purga semántica.** `etl_purga.py` normaliza el contrato canónico, convierte `activo` a booleano, mueve los campos filtrables a `metadatos`, resuelve la colisión de IDs y vectoriza los documentos para comparar distancia coseno. Con umbral `0.15`, elimina los casi-duplicados documentados en `docs-resultados/resultados_etl.md` y explica por qué `SELECT DISTINCT` no los habría detectado.

**B.6 — Killer Queries.** Corridas en `killer_queries.py` y documentadas en `docs-resultados/resultados_killer_queries.md`. La tercera consulta no se marca como aprobada: el falso positivo supera el umbral y queda registrado como limitación real del corpus, tal como exige la consigna.
...
**C.2 — El umbral de aceptación.** Oficial: **0,35** de similitud coseno, justificado en `docs-resultados/resultados_killer_queries.md` con datos reales (relevantes ≥0,39, fuera de dominio <0,30 la mayoría). La regla de aceptación queda definida como una doble validación: la consulta debe tener vocabulario del dominio EcoLogix y el mejor resultado debe superar el umbral antes de considerarse respuesta válida. En la práctica, la búsqueda `BaseVectorial.buscar(...)` rechaza automáticamente consultas fuera de dominio y descarta resultados por debajo de `0,35`; si no queda ninguna coincidencia aceptada, la salida es vacía y el sistema responde `no tengo esa información`, sin forzar el resultado más cercano. La validación real ejecutada con la consulta relevante (`"Necesito packaging para mandar comida caliente a domicilio, que no se rompa ni se moje"`) devolvió `doc-005` con `0,585`, mientras que la consulta fuera de dominio (`"Venden celulares o accesorios de telefonia?"`) devolvió `[]` y `LEN 0`, confirmando que la regla C.2 está funcionando. (`pipeline_vectorial.py`/FAISS sigue usando `umbral=0.25`, un valor distinto porque se recalibró para ese índice específico y no para la recuperación híbrida del dominio.)
